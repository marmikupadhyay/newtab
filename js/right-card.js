...
const fillTodos = () => {
  ...
  return;
	...
	$todo_box.style.maxHeight = '90%';
	$todo_box.innerHTML = '';
	$todo_box.id = 'todo-box';
	const $todos_list = document.createElement('div');
	$todos_list.id = 'todos-list';
	$todos_list.classList = 'max-h-full mb-2 oveflow-x-hidden overflow-y-auto';
	let unfinishedTodos = [],
		finishedTodos = [];
	todos.forEach((todo, index) => {
		const $todo_item = document.createElement('div');
		$todo_item.classList = `todo-item flex items-center p-2 ${
		...
		$todo_item.innerHTML = `
            ...
            <div class="w-1/3">${
				todo.deadline == undefined ? 'Not Set' : todo.deadline
			}</div>
            <button class="close-btn rounded-lg p-1" onClick="deleteTodo(${
				todo.id
			})">
	...
	$todo_form.innerHTML = `
        <input
            class="w-full rounded-lg p-3 py-2 border-t mr-0 border-b border-l
            border-r text-gray-800 border-gray-200
            transform bg-white"
            id="todo-input"
	...
	$todo_form.querySelector('input').addEventListener('keydown', (e) => {
		if (e.keyCode == 13)
			addNewTodo($todo_form.querySelector('input').value);
	});
